...
console.log('\nСверка сетки возврата');
{
  ...
  ok(/\(x\.p - m\) \* x\.n \/ total/.test(rc), 'ошибка использует веса того же среднего по наблюдениям');
  ok(/parts\.length < MIN_COINS/.test(rc), 'по горстке монет выводов не делаем');
  // Точки свёрнуты в счётчики по клеткам: держать под сотню тысяч объектов
  // рядом с торговым процессом на боевой машине — плохая идея.
  ok(/g\.n < MIN_PER_COIN/.test(rc), 'и по горстке точек внутри монеты тоже');
  ok(/delete cache\[coin\]/.test(rc), 'свечи монеты отпускаются сразу после подсчёта');
  // У зашитого числа ошибка тоже есть: считать его точным — значит объявлять
  // значимым любое расхождение
  ok(/Math\.SQRT2 \* m\.se/.test(rc) && !/S\.se\[cellKey\]/.test(rc), 'старая ошибка других весов не используется как проверенная');
  ...
  ok(/if \(got\.complete && historyReady\(merged, downloadFrom, to, true\)\)/.test(rc), 'плохая закачка не затирает хороший кеш');
  ...
  ok(/incomplete \? 'incomplete'/.test(rc) && /process\.exitCode = 2/.test(rc), 'неполная сверка не выдаётся за успех');
  const windows = read(path.join('src', 'recovery', 'recheck.js'));
  ok(/t0 - c\.t <= 86400/.test(windows), 'суточное окно считается по времени');
  ok(/future\.some\(\(c, j\) => c\.t !== t0 \+ \(j \+ 1\) \* 300\)/.test(windows), 'часовой возврат требует полный час');
  ...
  // Замер живёт на боевой машине рядом с торговым процессом. Без потолка
  // памяти жертву при нехватке выбирает система — и ею может оказаться сервер.
  ok(/spawn\(process\.execPath,\s*\n?\s*\['--max-old-space-size=320', 'scripts\/recheck-recovery\.js'\]/.test(src),
    'считает отдельный процесс с потолком памяти, а не торговый цикл');
  // Упавший замер и найденное расхождение — разные события. Слать про первое
  // «сетка разошлась с рынком» значит приучать не верить сообщениям.
  ok(/const drifted = \/РАЗОШЛОСЬ\/\.test\(out\)/.test(src),
    'упавший замер и расхождение — разные сообщения');
  ok(/сбой самого замера/.test(src), 'и про сбой сказано прямо');
  // Сообщение про расхождение говорило «панель обещает не то, что
  // происходит». С переходом панели на свежие наблюдения это перестало быть
  // правдой: сетку как оценку она не берёт, при нехватке данных ставит
  // прочерк. Зашитые проценты остались только в «обещано» журнала входов —
  // про него и надо писать. Неверная тревога обесценивает верную.
  {
    const msg = src.slice(src.indexOf('const drifted ='), src.indexOf('Сверка сетки не доработала'));
    ok(!/Панель обещает не то/.test(msg), 'не приписываем панели обещание, которого она не даёт');
    ok(/журнал/i.test(msg), 'названо место, где сетка ещё работает прогнозом');
    ok(/автоматически нельзя/.test(msg), 'и что подменять сетку измеренным нельзя');
  }
  ok(/if \(notify && code !== 0\) \{[\s\S]{0,200}sendTelegram/.test(src), 'разошлось — приходит сообщение, кроме явно тихой проверки');

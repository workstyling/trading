...
console.log('\nДесктоп: одна кнопка, работающая по числам из строки');
{
  ...
  // Считаем ПОДПИСЬ кнопки, а не слова: по всему файлу «Sell Stop» встречается
  // и в комментариях, и такая проверка ломалась от каждого разбора прошлой
  // ошибки рядом с кодом.
  const n = (h.match(/>Sell Stop</g) || []).length;

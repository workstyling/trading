...
for (const [name, file] of [['десктоп', 'public/index.html'], ['мобильная', 'public/mobile/index.html']]) {
  ...
  ok(/журнал проверяет первое появление сигнала/.test(src), 'длительность серии не выдаётся за проверенную пользу');
  // Уровень не должен читаться как обещание прибыли
  ok(/не вероятность чистой прибыли/.test(src), 'историческая частота не выдаётся за чистую прибыль');
  ok(f(row({ chg24Pct: null })).tier === 0, 'неизвестный ход за сутки не разрешает брать');
  ok(f(row({ pullbackPct: null })).tier === 0, 'неизвестный откат не разрешает брать');
}

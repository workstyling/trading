...
console.log('\nРазметка кнопок');
{
  ...
  // Объём — продаваемый, а не выбранный в таблице: свободного может быть
  // меньше, и тогда подпись под полями считала бы не тот ордер, который уйдёт.
  ok(/bumpSellStep\('\$\{coin\}','\$\{pair\}','stop',-1,'\$\{sellNow\}',\$\{sellNowCost\}\)/.test(h),
    'кнопки несут продаваемый объём и его долю затрат');
  ...
  // Подпись кнопки, а не слова: в комментариях «Sell Stop» тоже встречается.
  ok((h.match(/>Sell Stop</g) || []).length === 1, 'кнопка стоп-лимита ровно одна');
}

...
console.log('\nТаблица влезает в телефон');
{
  ...
  for (const c of ['монета', 'цель 1ч · группа', 'откат', 'за сутки']) ok(head.includes(c), 'колонка «' + c + '» на месте');
  ...
  ok(labels.join('|') === 'монета|цель 1ч · группа|откат|за сутки',
  ...
  ok(/'от пика ' \+/.test(cell) && /'спред ' \+/.test(cell), 'условия падения и спреда под монетой');
  ok(!/'вход ' \+/.test(cell), 'устаревший балл входа не выглядит разрешением');
  ok(/const sub = \[[\s\S]{0,400}held/.test(cell), 'срок в списке тоже');
  // Свежее пересечение видно и на телефоне: подсказок там нет
  ok(/recoveryFresh\(x\)[\s\S]{0,120}свежее/.test(cell), 'свежий вход помечен словом, а не только цветом');

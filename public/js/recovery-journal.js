(function (root) {
  ...
  // Пометка «максимум» снята с показа: см. recoveryOrder — выше частота не
  // лучше, а на четырёх часах хуже, так что зелёный значок на самой частой
  // монете указывал бы на худшую. Расчёт оставлен: он нужен подписи о том,
  // что верхние строки неразличимы, и проверкам.
  ...
  function recoveryOrder(row, scan, verdict, observation, now = scanNow(scan)) {
    ...
    // ПОРЯДОК НЕ ПО ЧАСТОТЕ.
    //
    // Сортировать по ней было первым побуждением, и проверка вне выборки его
    // отменила: на горизонте четырёх часов верхняя пятина по частоте дала
    // -0.332% против -0.143% у нижней (разница -0.188 ±0.064). Монета
    // касается цели чаще потому, что сильнее дёргается, — и чаще достаёт до
    // стопа. Список, отсортированный по частоте, показывал бы на худшее.
    //
    // Порядок теперь событийный, как у ленты: свежие пересечения сверху,
    // дальше по давности. Это не ранжирование по качеству, и панель так и
    // говорит — измеренного «лучше» у нас нет.
    const age = finite(row.inListMin) ? Math.max(0, Number(row.inListMin)) : 1e6;
    return priority + (recoveryFresh(row) ? 500 : 0) + 1 / (1 + age / 60);
  }

...
function calcScalpScore(s, vol24, spreadPct) {
  ...
  const checks = [
    ...
    //
    // Отсутствие истории считалось разрешением: `runUp24 == null || ...`
    // пропускало монету, чей суточный рост не удалось загрузить, — то есть
    // ровно тот случай, когда осторожность нужнее всего. Обе соседние проверки
    // (диапазон 4ч и вершина 14д) при нехватке данных отказывают, и в AGENTS.md
    // это записано отдельным правилом. Здесь была единственная брешь.
    { k: 'Не после пампа (рост ≤15%)', en: 'Not after a pump (24h run-up <=15%)', ok: s.runUp24 != null && s.runUp24 <= 15, v: s.runUp24 != null ? (s.runUp24 >= 0 ? '+' : '') + s.runUp24 + '%' : 'нет данных' },
